...
import { faHotel, faBook, faUsers, faCog } from '@fortawesome/free-solid-svg-icons';
import './Admin.css';
...
function Admin() {
    ...
    const handleSubmit = async (e) => {
        e.preventDefault();
        const url = editMode ? `${backend}api/hotels/update/${editId}` : `${backend}api/hotels/add`;
        ...
        for (const key in formData) {
            if (Array.isArray(formData[key])) {
                ...
                Array.from(formData.images).forEach(image => formDataToSend.append('images', image));
            } else {
            ...
            <div className="amenities-card amenities">
                ...
                {formData.amenities.includes('Free Wi-Fi') && (
                    <div className="amenity">
                        <img src={FreeWifiIcon} width="24px" height="24px" alt="Free Wi-Fi" />
                    </div>
                )}
        ...
        <div className={`d-flex ${sidebarVisible ? 'toggled' : ''}`} id="wrapper">
            ...
            <div className="bg-light border-right" id="sidebar-wrapper">
                ...
                <Nav className="list-group list-group-flush">
                    <Nav.Link
                        className={`list-group-item list-group-item-action bg-light ${activeLink === 'Hotels' ? 'active' : ''}`}
                        onClick={() => handleNavClick('Hotels')}
                    >
                        <FontAwesomeIcon icon={faHotel} className="mr-2" /> Hotels
                    </Nav.Link>
                    <Nav.Link
                        className={`list-group-item list-group-item-action ${activeLink === 'Bookings' ? 'active' : ''}`}
                        onClick={() => handleNavClick('Bookings')}
                    >
                        <FontAwesomeIcon icon={faBook} className="mr-2" /> Bookings
                    </Nav.Link>
                    <Nav.Link
                        className={`list-group-item list-group-item-action ${activeLink === 'Users' ? 'active' : ''}`}
                        onClick={() => handleNavClick('Users')}
                    >
                        <FontAwesomeIcon icon={faUsers} className="mr-2" /> Users
                    </Nav.Link>
                    <Nav.Link
                        className={`list-group-item list-group-item-action ${activeLink === 'Settings' ? 'active' : ''}`}
            ...
            <div id="page-content-wrapper">
                ...
                <Container fluid>
                    ...
                    <div id="hotel-list">
                        {hotels.map(hotel => (
                            <div className="card mb-3 hotel" key={hotel._id}>
                                <div className="card-body">
                                    <div className='hotel-info'>
                                        ...
                                        <div className='location-room'>
                                            <div className='location-price'>
                                                ...
                                                <p className="card-text">
                                                    <p>Location: </p> <small>{hotel.location.address}, {hotel.location.city}</small><p> {hotel.location.state}, {hotel.location.country}</p>
                                                </p>
                                                <p className="card-text">
                                                    <p>Pricing: </p><small>${hotel.pricing.minPricePerNight} - ${hotel.pricing.maxPricePerNight} per night</small>
                                                </p>
                                            </div>
                                            <div className='room-available'>
                                                <p className="card-text room-types">
                                                    <p>Room Types: </p><small>{hotel.roomTypes.join(', ')}</small>
                                                </p>
                                                <p className="card-text">
                                                    <p> Availability: </p><small>{hotel.availability.numberOfRoomsAvailable} rooms available</small>
                                                </p>
                                            </div>
                                        </div>
                                        ...
                                        <button className="btn btn-dark mr-2" onClick={() => handleEdit(hotel)}>Edit</button>
                                        <button className="btn btn-danger" onClick={() => handleDelete(hotel._id)}>Delete</button>

                                    </div>
                                    <div className='card-hotel-image'>
                                        {hotel.images.map((image, index) => (
                                            <img key={index} src={image.imageBase64} alt={image.filename} />
                                        ))}
                                    </div>
                                </div>
